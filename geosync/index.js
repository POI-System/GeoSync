...
function bindSocket(io) {
    ...
    io.on('connection', socket => {
        ...
        socket.on('geosync:join', async payload => {
            try {
                const { openId, adminToken, role } = payload || {};
                const scenicId = payload?.scenicId || CONFIG.scenicId;
                const rooms = [];
                if (role === 'admin') {
                    if (adminToken !== CONFIG.adminToken) {
                        return socket.emit('geosync:joined', { ok: false, message: '管理员令牌无效' });
                    }
...
function attach({ app, io, mongoose = require('mongoose'), models = {}, helpers = {} }) {
    if (attached) return;
    attached = true;
    ...
    startSseLoops();
    startJobs();
    walkGraph.loadIntoMemory().catch(e =>
        console.error('[GeoSync] [GRAPH] load failed (planner 将返回 1201):', e.message));
    crowdService.refreshPoiIndex().catch(e =>
        console.error('[GeoSync] [POI] index load failed:', e.message));

    console.log('[GeoSync] attached — routes/socket/jobs ready');
}
