...
async function initMap() {
    ...
    try {
        ...
        const roadNetwork = roadNetworkData || { type: 'FeatureCollection', features: [] };
        mapFacade.setRoadNetwork(roadNetwork);
        elements.map.dataset.roadCount = String(roadNetwork.features?.length || 0);
        const state = store.getState();
...
async function boot() {
    ...
    const results = await Promise.allSettled([
        ...
        typeof api.getRoadNetwork === 'function' ? api.getRoadNetwork() : Promise.resolve(null)
    ]);
    if (pageDestroyed) return;
    ...
    roadNetworkData = results[5].status === 'fulfilled' ? results[5].value : null;
