...
async function planFourHourPhotoShade(page, url = '/tour?demo=1') {
    ...
    await expect(page.locator('#road-banner')).toBeHidden();
...
async function startAndWaitForProposal(page) {
    ...
    await expect(page.locator('#road-banner')).toContainText('道路临时关闭（边 7）', { timeout: 3000 });
    await expect(page.locator('#road-banner')).toBeVisible();
...
test('runs the full plan, pause, resume, skip, reject, finish, and refresh workflow', async ({ page }) => {
    ...
    await expect(page.locator('#proposal-after-stops li')).toHaveText(['樱顶摄影点', '老图书馆', '珞珈湖步道']);
...
test('accepts a proposal and renders the complete returned route, ETA, stops, and version', async ({ page }) => {
    ...
    expect(accepted.stops.length).toBe(proposed.stops.length);
    ...
    expect(proposed.pendingProposal.beforeRoute.gis.source).toBe('demo-topology');
    expect(proposed.pendingProposal.afterRoute.gis.source).toBe('demo-topology');
    expect(proposed.pendingProposal.beforeRoute.sourceEdgeIds).toContain('7');
    expect(proposed.pendingProposal.afterRoute.sourceEdgeIds).not.toContain('7');
    expect(accepted.route.sourceEdgeIds).not.toContain('7');
    expect(accepted.route.geometry.coordinates.length).toBeGreaterThan(accepted.stops.length + 1);
...
for (const gisFailure of [
    ...
    { code: 8204, message: '当前路线模式没有可达路径，请调整路线偏好后主动重试' },
